...
const int sensitivity = 270;  // Quanto maior o valor = mais lento o mouse (em torno de 250)
int invertMouse = -1;         // Inversor de orientação do joystick, -1 para não inverter, 1 para inverter
 
 
char aux ="teste"; // Declaração da variável de auxílio
...
const char *mouse_com [3][3]{ //matriz de controle do mouse
  {"\0", "M_UP" ,"M_DOWN" },
  {"M_FRONT", "D_FRONT_UP", "D_FRONT_DOWN"},
  {"M_BACK", "D_BACK_UP" , "D_BACK_DOWN"}
};
...
{ // Movimento do ponteiro do mouse
  ...
  else if ((digitalRead(selPin)) && (mouseClickFlag)) // Se o botão não está pressionado (não sei se será usado)
    mouseClickFlag = 0;
 ...
 { // Garante que haverá navegação sem atraso do joystick que controle o mouse enquanto não houver nada sendo aplicado no joystick comando
  aux = mouse_com[horzValue2][vertValue2]; // Coloca em aux o valor da matriz que armazena os comandos e sua localização
  Mouse.press(aux); // Pressiona o comando desejado armazenado em aux
  delay(200); // Delay para evitar que haja uma repetição excessiva do comando
  Mouse.release(aux); // Solta o comando (senão fica pressionado para sempre)
